...
DIRS = PlatformDirs(appname=PLUGIN_NAME, appauthor=False)
...
class BromaCodegen:
    """Header file writer codegen for BromaIDA."""
    ...
    def write(self) -> Path:
        """
        Dumps codegen'd classes, structs and enums
        to the `PlatformDirs.user_cache_path` as a '.hpp' file
        ...
        self._file_path = (
            self._output_path
            / f"{self._target_platform}-{binary_name}.hpp"
        )
        ...
        with open(
            ...
            # reoccuring RobTop class members together, alongside other common types
            self._copy_content(f, "helpers.hpp")
            f.flush()

            # Set up ready-made definitions for the header parser
